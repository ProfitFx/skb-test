...
/**
  * Created by smakhetov on 07.09.2016.
  */


class ReportTest extends FreeSpecWithBrowserScaledScreen{
  "JsonPrint" in {
    ...
    val jsonParse = parse(inJson)
    createJsonFileToReport(jsonParse,"Тело ответного сообщения")
    val excelFileName = s"$downloadDir/Черновик подтверждения заказа №NN04 от 04.12.2011.xlsx"
    markup(s"""<a href='../$excelFileName'>excel file</a>""")
  }
}
